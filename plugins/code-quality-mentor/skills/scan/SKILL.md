...
7. **Run the LLM antipattern scan.** Skip if step 6 chose "Skip". Otherwise:
   - Read `${CLAUDE_SKILL_DIR}/assets/antipatterns.json` and `${CLAUDE_SKILL_DIR}/references/llm_scan_prompt.md`. The prompt file is the canonical specification of how you (Claude) carry out the scan; follow it.
...
- `${CLAUDE_SKILL_DIR}/assets/pmd-languages.json` — the Linguist→PMD language map.
- `${CLAUDE_SKILL_DIR}/assets/antipatterns.json` — the catalog of antipatterns and code smells the LLM scan applies.
- `${CLAUDE_SKILL_DIR}/references/slop-checks.md` — the slop carve-out applied as a post-pass to the freshly written LEARNING_PLAN.md.
- `${CLAUDE_SKILL_DIR}/references/llm_scan_prompt.md` — the prompt template that defines the LLM scan's procedure and output contract.
